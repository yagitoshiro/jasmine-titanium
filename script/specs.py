...
def sdk_version():
    tiapp_path = os.path.join(project_dir(), "tiapp.xml")
    tiapp_file = minidom.parse(tiapp_path)
    version_dom = tiapp_file.getElementsByTagName("sdk-version")[0]
    version = version_dom.childNodes[0].data
    return version
